...
class Note:
    '''Represent a note in the notebook. Macth against a string in searches and store tags for each note.'''
...
class Notebook:
    ...
    def modify_tag(self, note_id, tags):
		'''Find the note with the given if=d and change its tags to the given value'''
		for note in self.notes:
			if note.id == note_id:
				note.tags = tags
				break
